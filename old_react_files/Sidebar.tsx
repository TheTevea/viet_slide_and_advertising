...
export const Sidebar: React.FC<SidebarProps> = ({ activeCategory, onCategoryChange, isSidebarOpen, setIsSidebarOpen }) => {
    const [openMenus, setOpenMenus] = useState({ 
        mobile: activeCategory.startsWith('mobile'), 
        website: activeCategory.startsWith('website') 
    });
    ...
    return (
        <aside className={`w-72 bg-white border-r border-slate-200 flex flex-col h-screen fixed z-40 transition-transform duration-300 ease-in-out lg:translate-x-0 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
            ...
            <nav className="flex-1 p-4 overflow-y-auto">
                ...
                >
                    <SubNavItem text="Slide Show" active={activeCategory === 'mobile-slideshow'} onClick={() => onCategoryChange('mobile-slideshow')} />
                    <SubNavItem text="Advertising" active={activeCategory === 'mobile-advertising'} onClick={() => onCategoryChange('mobile-advertising')} />
                </NavItem>
                <NavItem
                ...
                >
                    <SubNavItem text="Slide Show" active={activeCategory === 'website-slideshow'} onClick={() => onCategoryChange('website-slideshow')} />
                    <SubNavItem text="Advertising" active={activeCategory === 'website-advertising'} onClick={() => onCategoryChange('website-advertising')} />
                </NavItem>
            </nav>
        </aside>
    );
